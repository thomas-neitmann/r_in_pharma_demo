    ...
    lapply(
      ...
      function(x){
        tibble(
          ...
          DSSDTC = c(
            as.POSIXct(x$RFSTDTC[1]) - lubridate::days(2),
            as.POSIXct(x$RFSTDTC[1]) - lubridate::days(1),
            as.POSIXct(x$RFSTDTC[1]),
            as.POSIXct(x$RFENDTC[1])
          )
...
saveRDS(DS, file = "data/sdtm/ds.rds")
